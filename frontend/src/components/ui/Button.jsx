export default function Button({ children, variant = "primary", className = "", ...props }) {
  const styles = {
    ...
    outline: "border border-brand-border bg-white text-brand-primary hover:border-blue-200 hover:bg-brand-goldSoft disabled:cursor-not-allowed disabled:opacity-50",
  ...
  return <button className={`inline-flex min-h-10 items-center justify-center rounded-xl px-4 py-2.5 text-sm font-bold transition ${styles[variant]} ${className}`} {...props}>{children}</button>;
}
