...
export default function OverheadAdd() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-5 md:p-6">
      <h2 className="text-xl font-bold">Add Overhead</h2>
      <p className="text-sm text-brand-muted">Track company operating costs outside direct project expenses.</p>
      {status === "fallback" && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-brand-danger">Could not load overhead categories. You can still save a manual category.</p>}
      ...
      <form onSubmit={submitOverhead} className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="lg:col-span-2"><Button disabled={status === "loading"}>Save Overhead</Button></div>
      </form>
    </Card>
  </div>;
}
