...
export default function PaymentEdit() {
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-5 md:p-6">
      ...
      <p className="text-sm text-brand-muted">Update client receipts and supplier payment records.</p>
      {status === "fallback" && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-brand-danger">Could not load this payment. Please check the backend and try again.</p>}
      <form onSubmit={submitPayment} className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="lg:col-span-2"><Button disabled={status !== "connected"}>Update Payment</Button></div>
      </form>
    </Card>
  </div>;
}
